...
function ImageryLayer(imageryProvider, options) {
  ...
  this.brightness = defaultValue(
    options.brightness,
    defaultValue(imageryProvider.defaultBrightness, ImageryLayer.DEFAULT_BRIGHTNESS)
  );
  ...
  this.saturation = defaultValue(
    options.saturation,
    defaultValue(imageryProvider.defaultSaturation, ImageryLayer.DEFAULT_SATURATION)
  );
  ...
  this.minificationFilter = defaultValue(
    ...
    defaultValue(imageryProvider.defaultMinificationFilter, ImageryLayer.DEFAULT_MINIFICATION_FILTER)
  );
  ...
  this.magnificationFilter = defaultValue(
    ...
    defaultValue(imageryProvider.defaultMagnificationFilter, ImageryLayer.DEFAULT_MAGNIFICATION_FILTER)
  );
...
ImageryLayer.prototype._createTileImagerySkeletons = function (tile, terrainProvider, insertionPoint) {
  ...
  if (
    Math.abs(northwestTileRectangle.south - tile.rectangle.north) < veryCloseY &&
  ...
  if (
    Math.abs(southeastTileRectangle.north - tile.rectangle.south) < veryCloseY &&
  ...
  for (var i = northwestTileCoordinates.x; i <= southeastTileCoordinates.x; i++) {
    ...
    if (
      ...
      (this.isBaseLayer() || Math.abs(clippedImageryRectangle.east - terrainRectangle.east) < veryCloseX)
    ) {
      maxU = 1.0;
    }
    ...
    for (var j = northwestTileCoordinates.y; j <= southeastTileCoordinates.y; j++) {
      ...
      if (
        ...
        (this.isBaseLayer() || Math.abs(clippedImageryRectangle.south - terrainRectangle.south) < veryCloseY)
      ) {
        minV = 0.0;
      }
...
function getLevelWithMaximumTexelSpacing(layer, texelSpacing, latitudeClosestToEquator) {
  ...
  var latitudeFactor = !(layer._imageryProvider.tilingScheme.projection instanceof GeographicProjection)
