...
function writeCompressedAttrib0(billboardCollection, context, textureAtlasCoordinates, vafWriters, billboard) {
  ...
  billboardCollection._allHorizontalCenter =
    billboardCollection._allHorizontalCenter && horizontalOrigin === HorizontalOrigin.CENTER;
  billboardCollection._allVerticalCenter =
    billboardCollection._allVerticalCenter && verticalOrigin === VerticalOrigin.CENTER;
...
function writeCompressedAttrib2(billboardCollection, context, textureAtlasCoordinates, vafWriters, billboard) {
  ...
  var validAlignedAxis =
    Math.abs(Cartesian3.magnitudeSquared(billboard.alignedAxis) - 1.0) < CesiumMath.EPSILON6 ? 1.0 : 0.0;
...
function writeCompressedAttribute3(billboardCollection, context, textureAtlasCoordinates, vafWriters, billboard) {
  ...
  var clampToGround =
    billboard.heightReference === HeightReference.CLAMP_TO_GROUND && billboardCollection._scene.context.depthTexture;
  ...
  if (!defined(billboard._labelDimensions)) {
    ...
    imageHeight = Math.round(
      defaultValue(billboard.height, billboardCollection._textureAtlas.texture.dimensions.y * height)
    );
...
function writeSDF(billboardCollection, context, textureAtlasCoordinates, vafWriters, billboard) {
  ...
  var compressed1 =
    Color.floatToByte(outlineColor.alpha) * LEFT_SHIFT16 + Color.floatToByte(outlineDistance) * LEFT_SHIFT8;
...
function updateMode(billboardCollection, frameState) {
  ...
  if (
    ...
    (mode !== SceneMode.SCENE3D && !Matrix4.equals(modelMatrix, billboardCollection.modelMatrix))
  ) {
    billboardCollection._mode = mode;
