...
function Primitive(options) {
  ...
  if (
    ...
    (!defined(this.geometryInstances) || (Array.isArray(this.geometryInstances) && this.geometryInstances.length !== 1))
  ) {
...
function modifyForEncodedNormals(primitive, vertexShaderSource) {
  ...
  if (containsNormal && containsTangent && containsBitangent) {
    ...
    decode +=
      "    czm_octDecode(" + attributeName + "." + (containsSt ? "yz" : "xy") + ", normal, tangent, bitangent);\n";
  } else {
    if (containsNormal) {
      globalDecl += "vec3 normal;\n";
      decode +=
    ...
    if (containsTangent) {
      ...
      decode +=
        "    tangent = czm_octDecode(" + attributeName + "." + (containsSt && containsNormal ? "z" : "y") + ");\n";
    }

    if (containsBitangent) {
      globalDecl += "vec3 bitangent;\n";
      decode +=
        "    bitangent = czm_octDecode(" + attributeName + "." + (containsSt && containsNormal ? "z" : "y") + ");\n";
    }
  }
