      ...
      for (var ii = 0; ii < length; ++ii) {
        ...
        if (this._batchTable.attributes.length > 2) {
          if (properties[POSITION_INDEX] || properties[POSITION_SIZE_INDEX]) {
            var boundingSphere =
              frameState.mode === SceneMode.SCENE2D ? polyline._boundingVolume2D : polyline._boundingVolumeWC;
    ...
    for (var k = 0; k < numberOfIndicesArrays; ++k) {
      ...
      if (indices.length > 0) {
        ...
        var positionHighOffset =
          6 * (k * (positionSizeInBytes * CesiumMath.SIXTY_FOUR_KILOBYTES) - vbo * positionSizeInBytes); //componentsPerAttribute(3) * componentDatatype(4)
        ...
        var vertexTexCoordExpandAndBatchIndexBufferOffset =
          k * (texCoordExpandAndBatchIndexSizeInBytes * CesiumMath.SIXTY_FOUR_KILOBYTES) -
