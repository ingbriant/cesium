...
function visitRenderedTiles(
  ...
  while (
    ...
    (tile._lastSelectionResultFrame !== currentFrameNumber ||
      ...
      TileSelectionResult.originalResult(tile._lastSelectionResult) === TileSelectionResult.CULLED)
  ) {
...
function propagateEdge(tileProvider, frameState, sourceTile, destinationTile, tileEdge, vertexArraysToDestroy) {
  ...
  switch (tileEdge) {
    ...
    case TileEdge.NORTHWEST:
      destinationFill.changedThisFrame =
        destinationFill.changedThisFrame || destinationFill.northwestMesh !== sourceMesh;
      destinationFill.northwestMesh = sourceMesh;
      destinationFill.northwestTile = sourceTile;
      return;
    case TileEdge.NORTHEAST:
      destinationFill.changedThisFrame =
        destinationFill.changedThisFrame || destinationFill.northeastMesh !== sourceMesh;
      destinationFill.northeastMesh = sourceMesh;
      destinationFill.northeastTile = sourceTile;
      return;
    case TileEdge.SOUTHWEST:
      destinationFill.changedThisFrame =
        destinationFill.changedThisFrame || destinationFill.southwestMesh !== sourceMesh;
      destinationFill.southwestMesh = sourceMesh;
      destinationFill.southwestTile = sourceTile;
      return;
    case TileEdge.SOUTHEAST:
      destinationFill.changedThisFrame =
        destinationFill.changedThisFrame || destinationFill.southeastMesh !== sourceMesh;
  ...
  if (sourceTile.level <= destinationTile.level) {
    ...
    destinationFill.changedThisFrame =
      destinationFill.changedThisFrame || edgeMeshes[0] !== sourceMesh || edgeMeshes.length !== 1;
...
function createFillMesh(tileProvider, frameState, tile, vertexArraysToDestroy) {
  ...
  if (rectangle.width > maxTileWidth && maximumHeight - minimumHeight <= geometricError) {
    ...
    var oneOverMercatorHeight =
      1.0 / (WebMercatorProjection.geodeticLatitudeToMercatorAngle(rectangle.north) - southMercatorY);
    var centerWebMercatorT =
      (WebMercatorProjection.geodeticLatitudeToMercatorAngle(centerCartographic.latitude) - southMercatorY) *
...
function transformTextureCoordinates(sourceTile, targetTile, coordinates, result) {
  ...
  if (
    targetTile.x === 0 &&
    coordinates.x === 1.0 &&
    sourceTile.x === sourceTile.tilingScheme.getNumberOfXTilesAtLevel(sourceTile.level) - 1
  ) {
    ...
    sourceTile.x === 0 &&
    coordinates.x === 0.0 &&
    targetTile.x === targetTile.tilingScheme.getNumberOfXTilesAtLevel(targetTile.level) - 1
  ) {
    sourceRectangle = Rectangle.clone(sourceTile.rectangle, sourceRectangleScratch);
    sourceRectangle.west += CesiumMath.TWO_PI;
    sourceRectangle.east += CesiumMath.TWO_PI;
...
function getInterpolatedVertexAtCorner(
  ...
  var previousUv = transformTextureCoordinates(
    sourceTile,
    targetTile,
    sourceEncoding.decodeTextureCoordinates(sourceVertices, previousIndex, uvScratch),
    uvScratch
  );
  var nextUv = transformTextureCoordinates(
    sourceTile,
    targetTile,
    sourceEncoding.decodeTextureCoordinates(sourceVertices, nextIndex, uvScratch2),
    uvScratch2
  );
...
function addEdgeMesh(
  ...
  if (sourceEncoding.hasWebMercatorT) {
    ...
    oneOverMercatorHeight =
      1.0 / (WebMercatorProjection.geodeticLatitudeToMercatorAngle(targetRectangle.north) - southMercatorY);
  }
  ...
  for (var i = 0; i < indices.length; ++i) {
    ...
    if (sourceEncoding.hasWebMercatorT) {
      ...
      webMercatorT =
        (WebMercatorProjection.geodeticLatitudeToMercatorAngle(latitude) - southMercatorY) * oneOverMercatorHeight;
    }
