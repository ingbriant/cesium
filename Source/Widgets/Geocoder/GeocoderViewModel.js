...
function GeocoderViewModel(options) {
  ...
  if (defined(options.geocoderServices)) {
    ...
    this._geocoderServices = [new CartographicGeocoderService(), new IonGeocoderService({ scene: options.scene })];
  }
...
function flyToDestination(viewModel, destination) {
  ...
  if (destination instanceof Rectangle) {
    ...
    if (
      CesiumMath.equalsEpsilon(destination.south, destination.north, CesiumMath.EPSILON7) &&
