  ...
  while (newDataIndex < newData.length) {
    ...
    if (timesInsertionPoint < 0) {
      ...
      while (newDataIndex < newData.length) {
        currentTime = convertDate(newData[newDataIndex], epoch);
        if (
          (defined(prevItem) && JulianDate.compare(prevItem, currentTime) >= 0) ||
...
SampledProperty.prototype.getValue = function (time, result) {
  ...
  if (index < 0) {
    ...
    if (index === 0) {
      ...
      if (
        this._backwardExtrapolationType === ExtrapolationType.NONE ||
        (timeout !== 0 && JulianDate.secondsDifference(startTime, time) > timeout)
      ) {
        return undefined;
      }
    ...
    if (this._updateTableLength) {
      ...
      var numberOfPoints = Math.min(
        interpolationAlgorithm.getRequiredDataPoints(this._interpolationDegree, inputOrder),
        timesLength
      );
