...
function updateTransform(that, camera, updateLookAt, saveCamera, positionProperty, time, ellipsoid) {
  ...
  if (defined(cartesian)) {
    ...
    if (mode === SceneMode.SCENE3D) {
      ...
      if (defined(deltaCartesian)) {
        ...
        var semiMajorAxis =
          -mu / (inertialVelocity * inertialVelocity - (2 * mu) / Cartesian3.magnitude(inertialCartesian));
  ...
  if (updateLookAt) {
    var offset =
      mode === SceneMode.SCENE2D || Cartesian3.equals(that._offset3D, Cartesian3.ZERO) ? undefined : that._offset3D;
