...
PointVisualizer.prototype.update = function (time) {
  ...
  for (var i = 0, len = items.length; i < len; i++) {
    ...
    var show =
      entity.isShowing && entity.isAvailable(time) && Property.getValueOrDefault(pointGraphics._show, time, true);
    var position;
    if (show) {
    ...
    if (defined(pointPrimitive)) {
      ...
      var newOutlineWidth = Math.round(
        Property.getValueOrDefault(pointGraphics._outlineWidth, time, defaultOutlineWidth)
      );
      var newPixelSize = Math.max(
        1,
        Math.round(Property.getValueOrDefault(pointGraphics._pixelSize, time, defaultPixelSize))
      );
      ...
      if (needsRedraw) {
        ...
        billboard.setImage(
          textureId,
          createBillboardPointCallback(centerAlpha, cssColor, cssOutlineColor, newOutlineWidth, newPixelSize)
        );
      }
