...
function createPositionPropertyFromAltitudeMode(property, altitudeMode, gxAltitudeMode) {
  ...
  if (
    ...
    oneTimeWarning(
      "kml-altitudeMode-unknown",
      "KML - Unknown altitudeMode: " + defaultValue(altitudeMode, gxAltitudeMode)
    );
  }
    ...
    if (defined(href)) {
      ...
      if (/^data:/.test(href.getUrlComponent())) {
        ...
        var defaultViewFormat =
          viewRefreshMode === "onStop" ? "BBOX=[bboxWest],[bboxSouth],[bboxEast],[bboxNorth]" : "";
          ...
          if (
            ...
            if (hasNetworkLinkControl) {
              networkLinkInfo.cookie = queryToObject(
                defaultValue(queryStringValue(networkLinkControl, "cookie", namespaces.kml), "")
              );
              minRefreshPeriod = defaultValue(
                queryNumericValue(networkLinkControl, "minRefreshPeriod", namespaces.kml),
                0
              );
            }
...
function loadKml(dataSource, entityCollection, kml, sourceResource, uriResolver, context) {
  ...
  return when
    .all(processStyles(dataSource, kml, styleCollection, sourceResource, false, uriResolver))
    .then(function () {
    ...
    if (!networkLink.updating) {
      ...
      if (networkLink.refreshMode === RefreshMode.INTERVAL) {
        ...
        if (
          ...
          JulianDate.secondsDifference(now, networkLink.cameraUpdateTime) >= networkLink.time
        ) {
          doUpdate = true;
        }
      }
