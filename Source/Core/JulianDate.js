      ...
      if (tokens !== null) {
        ...
        if (tokens !== null) {
          ...
          if (tokens !== null) {
            ...
            if (
              dashCount > 0 &&
              ((!defined(tokens[3]) && dashCount !== 1) || (defined(tokens[3]) && dashCount !== 2))
            ) {
              throw new DeveloperError(iso8601ErrorMessage);
            }
            //>>includeEnd('debug')
...
JulianDate.toDate = function (julianDate) {
  ...
  return new Date(
    Date.UTC(gDate.year, gDate.month - 1, gDate.day, gDate.hour, gDate.minute, second, gDate.millisecond)
  );
};
...
JulianDate.equals = function (left, right) {
  return (
    left === right ||
    (defined(left) && defined(right) && left.dayNumber === right.dayNumber && left.secondsOfDay === right.secondsOfDay)
  );
};
...
JulianDate.equalsEpsilon = function (left, right, epsilon) {
  ...
  return (
    left === right ||
    (defined(left) && defined(right) && Math.abs(JulianDate.secondsDifference(left, right)) <= epsilon)
  );
};
