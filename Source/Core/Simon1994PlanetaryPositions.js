...
function meanAnomalyToEccentricAnomaly(meanAnomaly, eccentricity) {
  ...
  var iterationValue =
    meanAnomaly +
    (eccentricity * Math.sin(meanAnomaly)) / (1.0 - Math.sin(meanAnomaly + eccentricity) + Math.sin(meanAnomaly));
...
function computeSimonMoon(date, result) {
  ...
  var perigeeAndMean =
    2.116 * Math.sin(psi) * t - 0.111 * Math.sin(twoD - twoF - psi) * t - 0.0015 * Math.sin(psi) * t2;
