        ...
        if (JulianDate.greaterThan(interval.stop, intervals[index - 1].stop)) {
          ...
          interval = new TimeInterval({
            ...
            isStopIncluded:
              ...
              (interval.stop.equals(intervals[index - 1].stop) && interval.isStopIncluded),
            data: interval.data,
          });
        }
    ...
    if (comparison > 0 || (comparison === 0 && (interval.isStopIncluded || intervals[index].isStartIncluded))) {
      ...
      if (
        ...
        interval = new TimeInterval({
          ...
          isStopIncluded: JulianDate.greaterThan(intervals[index].stop, interval.stop)
...
function parseDuration(iso8601, result) {
  ...
  return (
    result.year || result.month || result.day || result.hour || result.minute || result.second || result.millisecond
  );
}
