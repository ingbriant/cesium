...
xdescribe(
  ...
  function () {
    ...
    it("renders batched polygons with a batch table", function () {
      scene.primitives.add(globePrimitive);
      return Cesium3DTilesTester.loadTileset(scene, vectorPolygonsBatchedChildrenWithBatchTable).then(function (
        tileset
      ) {
        verifyRender(tileset, scene);
        verifyPick(scene);
    ...
    it("renders batched polylines with a batch table", function () {
      return Cesium3DTilesTester.loadTileset(scene, vectorPolylinesBatchedChildrenWithBatchTable).then(function (
        tileset
      ) {
        verifyRenderPolylines(tileset, scene);
      });
