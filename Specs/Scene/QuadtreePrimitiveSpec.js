  ...
  describe("selectTilesForRendering", function () {
    ...
    it("selects deeper tiles once they are renderable", function () {
      ...
      rootTiles[0].children.forEach(function (tile) {
        mockTerrain.requestTileGeometryWillSucceed(tile).createMeshWillSucceed(tile);
    ...
    it("skips loading levels when tiles are known to be available", function () {
      ...
      rootTiles.forEach(function (tile) {
        // level 0 tile
        mockTerrain.willBeAvailable(tile).requestTileGeometryWillSucceed(tile).createMeshWillSucceed(tile);
    ...
    function () {
      ...
      it("renders tiles in approximate near-to-far order", function () {
        ...
        return pollToPromise(function () {
          ...
          expect(
            quadtree._tilesToRender[1] === east.southwestChild || quadtree._tilesToRender[1] === east.northeastChild
          ).toBe(true);
          expect(
            quadtree._tilesToRender[2] === east.southwestChild || quadtree._tilesToRender[2] === east.northeastChild
          ).toBe(true);
          expect(quadtree._tilesToRender[3]).toBe(east.southeastChild);
          expect(quadtree._tilesToRender[4]).toBe(west.northeastChild);
          expect(
            quadtree._tilesToRender[5] === west.northwestChild || quadtree._tilesToRender[5] === west.southeastChild
          ).toBe(true);
          expect(
            quadtree._tilesToRender[6] === west.northwestChild || quadtree._tilesToRender[6] === west.southeastChild
          ).toBe(true);
