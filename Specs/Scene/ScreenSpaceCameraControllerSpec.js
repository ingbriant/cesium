...
describe("Scene/ScreenSpaceCameraController", function () {
  ...
  it("looks in Columbus view", function () {
    ...
    expect(Cartesian3.cross(camera.direction, camera.up, new Cartesian3())).toEqualEpsilon(
      camera.right,
      CesiumMath.EPSILON12
    );
    expect(Cartesian3.cross(camera.up, camera.right, new Cartesian3())).toEqualEpsilon(
      camera.direction,
      CesiumMath.EPSILON12
    );
    expect(Cartesian3.cross(camera.right, camera.direction, new Cartesian3())).toEqualEpsilon(
      camera.up,
      CesiumMath.EPSILON12
    );
  });
  ...
  it("rotates in Columbus view", function () {
    ...
    moveMouse(MouseButtons.MIDDLE, startPosition, endPosition);
    updateController();
    expect(Cartesian3.dot(Cartesian3.normalize(camera.position, new Cartesian3()), Cartesian3.UNIT_Z)).toBeGreaterThan(
      0.0
    );
  ...
  it("pans with constrained axis other than z-axis", function () {
    ...
    expect(camera.right).toEqualEpsilon(
      Cartesian3.normalize(Cartesian3.cross(axis, camera.position, new Cartesian3()), new Cartesian3()),
      CesiumMath.EPSILON14
    );
