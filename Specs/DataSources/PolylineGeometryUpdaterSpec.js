...
describe(
  ...
  function () {
    ...
    function validateGeometryInstance(options) {
      ...
      if (clampToGround) {
        ...
        if (options.depthFailMaterial && options.depthFailMaterial instanceof ColorMaterialProperty) {
          expect(attributes.depthFailColor.value).toEqual(
            ColorGeometryInstanceAttribute.toValue(options.depthFailMaterial.color.getValue(time))
          );
        } else {
      ...
      if (options.material instanceof ColorMaterialProperty) {
        expect(attributes.color.value).toEqual(
          ColorGeometryInstanceAttribute.toValue(options.material.color.getValue(time))
        );
      } else {
      ...
      if (options.distanceDisplayCondition) {
        expect(attributes.distanceDisplayCondition.value).toEqual(
          DistanceDisplayConditionGeometryInstanceAttribute.toValue(options.distanceDisplayCondition)
        );
      }
    }
    ...
    it("Attributes have expected values at creation time", function () {
      ...
      expect(attributes.color.value).toEqual(
        ColorGeometryInstanceAttribute.toValue(colorMaterial.color.getValue(time2))
      );
    ...
    it("geometryChanged event is raised when expected", function () {
      ...
      entity.polyline.positions = new ConstantProperty(Cartesian3.fromRadiansArray([0, 0, 1, 0]));
