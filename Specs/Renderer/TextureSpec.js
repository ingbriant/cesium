...
describe(
  ...
  function () {
    ...
    beforeAll(function () {
      ...
      promises.push(
        Resource.fetchImage("./Data/Images/BlueAlpha.png").then(function (image) {
          blueAlphaImage = image;
        })
      );
      promises.push(
        Resource.fetchImage("./Data/Images/BlueOverRed.png").then(function (image) {
    ...
    it("can create a texture from the framebuffer", function () {
      ...
      expect(texture.sizeInBytes).toEqual(
        expectedWidth * expectedHeight * PixelFormat.componentsLength(texture.pixelFormat)
      );
    ...
    it("cannot flip texture when using ImageBitmap", function () {
      ...
      return Resource.supportsImageBitmapOptions().then(function (supportsImageBitmapOptions) {
    ...
    it("draws the expected floating-point texture color with linear filtering", function () {
      ...
      var fs = "uniform sampler2D u_texture;" + "void main() { gl_FragColor = texture2D(u_texture, vec2(0.5, 0.0)); }";
    ...
    it("draws the expected half floating-point texture color with linear filtering", function () {
      ...
      var color0 = new Color(0.2, 0.4, 0.6, 1.0);
      var color1 = new Color(0.1, 0.3, 0.5, 1.0);
      var floats = new Uint16Array([12902, 13926, 14541, 15360, 11878, 13517, 14336, 15360]);
