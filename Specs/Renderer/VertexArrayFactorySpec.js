  ...
  function () {
    ...
    it("creates a single-attribute vertex (interleaved)", function () {
      ...
      expect(va.getAttribute(0).strideInBytes).toEqual(
        ComponentDatatype.getSizeInBytes(position.componentDatatype) * position.componentsPerAttribute
      );
    ...
    it("creates a homogeneous multiple-attribute vertex (interleaved)", function () {
      ...
      var expectedStride =
        ComponentDatatype.getSizeInBytes(position.componentDatatype) * position.componentsPerAttribute +
        ComponentDatatype.getSizeInBytes(normal.componentDatatype) * normal.componentsPerAttribute;
    ...
    it("creates a heterogeneous multiple-attribute vertex (interleaved)", function () {
      ...
      var expectedStride =
        ComponentDatatype.getSizeInBytes(position.componentDatatype) * position.componentsPerAttribute +
        ComponentDatatype.getSizeInBytes(colors.componentDatatype) * colors.componentsPerAttribute;
    ...
    it("sorts interleaved attributes from large to small components", function () {
      ...
      var vs =
        ...
        "}";
      var fs = "varying vec4 fsColor; " + "void main() { " + "  gl_FragColor = fsColor; " + "}";
